...
__all__ = ["PreProcessor", "TrOCRPreProcessor"]
...
class TrOCRPreProcessor(nn.Module):
    ...
    def __call__(self, x:Union[list[str], torch.Tensor, np.ndarray]):
        ...
        if isinstance(x, (np.ndarray, torch.Tensor)):
            ...
            # Sample transform (to tensor, resize)
            samples = x #list(multithread_exec(self.sample_transforms, x))
            # Batching
            self.batch_size = min([len(x), 1])
            batches = self.batch_inputs(samples)
        elif isinstance(x, list) and all(isinstance(item, str) for item in x):
            if not all(os.path.isfile(filename) for filename in x):
                raise FileNotFoundError("Some or all files given are not found.")
            batches = x
        else:
            raise TypeError(f"invalid input type: {type(x)}")
