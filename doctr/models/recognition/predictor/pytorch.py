...
__all__ = ["RecognitionPredictor", "TrOCRPredictor"]
...
class TrOCRPredictor(nn.Module):
    
    def __init__(self, model) -> None:
        super().__init__()
        self.model = model
    
    def forward(self, x:Sequence[Union[np.ndarray, torch.Tensor]]):
        words_pred = [
            self.model(cropped_image) for cropped_image in x
        ]
        return words_pred